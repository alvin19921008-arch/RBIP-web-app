...
function SchedulePageContent() {
  ...
  const substitutionWizardDataForDisplay = useMemo(() => {
    ...
    teams.forEach((mainTeam) => {
      const contributors = teamContributorsByMain[mainTeam] || [mainTeam]
      substitutionsByTeam[mainTeam] = contributors.flatMap((team) =>
        (substitutionWizardData.substitutionsByTeam[team] || []).map((row) => ({
          ...row,
          team: mainTeam,
        }))
      )
    ...
    const initialSelections = substitutionWizardData.initialSelections
      ? Object.entries(substitutionWizardData.initialSelections).reduce<
          Record<string, Array<{ floatingPCAId: string; slots: number[] }>>
        >((acc, [key, selections]) => {
          const sourceTeam = TEAMS.find((team) => key.startsWith(`${team}-`))
          if (!sourceTeam) {
            acc[key] = selections
            return acc
          }
          const mainTeam = getMainTeam(sourceTeam, mergedInto)
          const nextKey = `${mainTeam}${key.slice(sourceTeam.length)}`
          const existing = acc[nextKey] || []
          const incoming = Array.isArray(selections) ? selections : []
          const merged = [...existing]
          incoming.forEach((item) => {
            const sig = `${item.floatingPCAId}:${(item.slots || []).slice().sort((a, b) => a - b).join(',')}`
            const hasDup = merged.some((m) => {
              const mSig = `${m.floatingPCAId}:${(m.slots || []).slice().sort((a, b) => a - b).join(',')}`
              return mSig === sig
            })
            if (!hasDup) merged.push(item)
          })
          acc[nextKey] = merged
          return acc
        }, {})
  ...
  const pcaAllocationsForDisplay = useMemo(() => {
    ...
    visibleTeams.forEach((mainTeam) => {
      const contributors = teamContributorsByMain[mainTeam] || [mainTeam]
      out[mainTeam] = contributors.flatMap((team) => pcaAllocationsForUi[team] || [])
    })
    return out
  }, [visibleTeams, teamContributorsByMain, pcaAllocationsForUi])
  ...
  const pcaOverridesByTeam = useMemo(() => {
    ...
    return next
  }, [visibleTeams, pcaAllocationsForDisplay, pcaAllocationsForUi, staffOverrides])

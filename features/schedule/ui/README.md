...
- **Remaining Phase 6 (planned):** peel the rest of the **Phase 4 inventory** allocation surface into `features/schedule/ui/` per `docs/superpowers/plans/2026-04-11-schedule-ui-lib-separation-plan.md` and the **implementation plan** companion (exit criteria, verification matrix). When dialog UI moves, resolve the `BedCountsOverrideState` type reference in `lib/features/schedule/controller/scheduleControllerTypes.ts` (extract shared types to `lib/` or `@/types/schedule`).

...
- **Still to migrate (Phase 6 — planned):** same peel as above is **tracked as Phase 6** in `docs/superpowers/plans/2026-04-11-schedule-ui-lib-separation-plan.md` and the **implementation plan** companion (exit criteria, verification matrix). Also includes moving **`step34ViewModel`** to **`steps/step3-floating/substeps/step34-preview/`** per **`ARCHITECTURE_ESSENTIALS.mdc`**. Resolve the `BedCountsOverrideState` type reference in `lib/features/schedule/controller/scheduleControllerTypes.ts` when dialog UI moves (extract shared types to `lib/` or `@/types/schedule`).

-- Update copy_schedule_v1 to support snapshot-local buffer staff exclusion.
--
-- Previous versions excluded buffer staff allocations by checking global staff.status = 'buffer'.
-- With snapshot-local buffer staff (stored under daily_schedules.staff_overrides.__staffStatusOverrides),
-- we pass buffer staff ids explicitly (buffer_staff_ids) and exclude allocations by staff_id.
--
-- NOTE: This is backward compatible for callers that don't pass buffer_staff_ids (default NULL).

-- Fix save_schedule_v1 schema drift for legacy PCA leave comeback columns.
--
-- Symptom:
--   RPC save fails with errors like:
--   column "leave_comeback_time" does not exist
--
-- Cause:
--   Function definition drifted to a legacy version that still references
--   schedule_pca_allocations.leave_comeback_time / leave_mode, while those
--   columns were removed from schema.
--
-- Fix:
--   Recreate save_schedule_v1 against current schema (no legacy PCA leave columns).

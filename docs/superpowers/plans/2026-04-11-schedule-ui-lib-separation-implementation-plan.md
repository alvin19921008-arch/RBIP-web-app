...
- [x] Remaining wizard UI lives under `features/schedule/ui/steps/**` per architecture **§B**. (Floating PCA config wizard + `step34ViewModel` → `steps/step3-floating/` in Phase 5a; optional follow-up: align `step34/` with `substeps/step34-preview/` naming.)

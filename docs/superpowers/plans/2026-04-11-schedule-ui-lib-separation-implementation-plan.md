...
4. For **Phases 3–6** (any phase that moves schedule UI), follow **§ Deferred → Required two-slice workflow**: land **structural** work first → **review + gate + smoke + manual** → then **token-alignment** commit(s) on touched UI → re-run gate — or log **token N/A (no UI diff)** explicitly; do not skip token silently.
...
| **6** | Yes | **`f47` + `f66`** when Step 3 / page import graph moves; full regression + grep hygiene for **`@/components/allocation`** + **token slice** after each peel verify (see **§ Phase 6**) | **Yes** before Phase 6 `done` |
...
#### Required two-slice workflow (Phases **3**, **4**, **5**, **6** — do not skip token slice)
...
- [x] New modules under `lib/features/schedule/controller/` (or adjacent); **public** `useScheduleController` API unchanged unless versioned in the **same commit/slice**. (`scheduleControllerTypes.ts`, `scheduleDomainState.ts`; further hook decomposition optional.)
...
- [x] Mandatory gate passes (after **structural** slice and again after **token** slice when applicable).
- [x] **Full** regression suite (`find … npx tsx` loop) passes. (`f124` was failing pre–Phase 3; fixed in `c2b11ea` without regressing Step 3 smoke.)
- [x] Manual: Step 2 run, Step 3 run, Step 4 bed relieving, save/load smoke path.
...
- [x] Dead hooks removed or wired; imports updated. (`hooks/useScheduleState.ts`, `hooks/useStepwiseAllocation.ts` removed.)
- [x] `components/schedule` empty or thin re-exports only. (Shims → `features/schedule/ui/**`; consumers use `@/features/schedule/ui/...`.)
- [x] **`components/allocation/`** — per architecture plan: inventory schedule-only surfaces; migrate or track in a checklist so Phase 4 is not “done” with allocation UI still orphaned outside `features/schedule/ui/`. (See `features/schedule/ui/README.md` Phase 4 subsection.)
...
- [x] Mandatory gate passes after structural slices and after token slices as applicable.
- [x] Full regression suite passes.
- [x] `grep -R "@/components/schedule" --include='*.tsx' --include='*.ts'` returns no stale paths (or only allowed shims). (No `@/components/schedule` in `*.ts`/`*.tsx`; shims re-export from `@/features/schedule/ui/...`.)
...
- [x] Remaining wizard UI lives under `features/schedule/ui/steps/**` per architecture **§B**. (Floating PCA config wizard + `step34ViewModel` → `steps/step3-floating/` in Phase 5a; **§B substep folder parity** for `step34` → **`substeps/step34-preview/`** is **Phase 6**.)
...
- [x] Mandatory gate + full regression + extended manual wizard walkthrough (Steps 1–5), **after structural and after token** slices as applicable. (Gate + full regression `tsx` loop green 2026-04-16; extended manual wizard: **solo acceptance** — confirm in tracker when humans sign off.)
...
- [ ] **Allocation peel (Bucket A + inventory):** Migrate remaining schedule-primary surfaces in **`features/schedule/ui/README.md`** from **`components/allocation/`** into **`features/schedule/ui/`** in **incremental slices** (notably **grid / `SchedulePageClient`**, **`PCABlock`** + **`pcaTracker/*`**, dialogs, popovers, etc.). **Bucket B** wizard subtree is **canonical under `steps/step3-floating/`** with **thin shims** only — do not treat `step3V2` / `step32V2` / team wizard cards as “still to peel” from allocation. Leave **thin shims** where required for stable paths or update non-schedule consumers in the same slice.
...
- [ ] Mandatory gate after each structural slice and after token commits when applicable.
- [ ] **`npx tsx tests/regression/f47-page-step3-runtime-uses-shared-builders.test.ts`** and **`f66-step3-v2-harness-uses-ranked-slot-engine.test.ts`** when Step 3 / schedule page import graph moves.
- [ ] **Full** `find tests/regression -name '*.test.ts' … npx tsx` loop before marking Phase **6** `done` for a milestone that touches allocation, save/load, or step runners.
- [ ] Manual: Steps **1–5** (and save/load) after slices that change user-visible schedule surfaces.
- [ ] Grep / README: track progress reducing **`@/components/allocation`** usage from schedule code to **shim-only** or zero for migrated symbols; document intentional exceptions in **`features/schedule/ui/README.md`**.

**Done when:** `substeps/step34-preview/` is the canonical home for Step 3.4 view-model code; README inventory updated for peeled vs remaining allocation components; mandatory gate + full regression + manual acceptance; token follow-up per **§ Deferred** completed for each peel (or **token N/A** logged per slice).

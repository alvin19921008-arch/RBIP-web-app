...
|-------|------------|---------|---------------------------|-------------------------|
| **Avg** | Canonical **raw** average PCA per team for display and for deriving floating need. One number reused across Step 1 / 2 / 3 surfaces that show “Avg PCA/team”. | `1.35` | **Produced:** Step 1 / Step 2 workflow (`calculations` / merged team targets). **Consumed:** dashboard, Step 3.1 card, bootstrap input for projection. | **Step 1–2** (leave, therapist, non-floating, special programs). **Not** edited in Step 3.1 except indirectly if surplus policy (below) has already folded raw surplus into this scalar at the Step 2→3 boundary. |

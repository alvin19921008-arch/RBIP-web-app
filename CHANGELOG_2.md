...
- **Step 2 → Step 3 handoff and save/reload** — `step3Bootstrap` delta for toast (`describeStep3BootstrapDelta`); `extraCoverageVisibility` (`shouldShowExtraCoverage`, `sanitizeExtraCoverageOverrides`); `pcaSubstitutionDisplay` (`derivePcaSubstitutionInfo`); `step2Point2StateMerge` (`mergeStep2Point2StaffOverrides`); `saveNormalization` (`normalizeScheduleStateForSave`); `saveReconciliation` (`computeStalePcaStaffIdsForReplace`). Regressions f39–f45 (substitution display, multi-team slot-teams, Step 3 excludes special-program reserved slots, Step 2.2 preserves Step 2.1 overrides, Robotic slot routing, save-cleared-Step2 allocations, RPC replaces stale PCA rows).
...
- **Step 2.2 → Step 3 handoff** — Step 2 success toast fires after Step 2.2 confirm, with Step 3 bootstrap summary (e.g. “Y team + X PCA slots”); toast supports longer duration, hover-to-pause, and countdown bar; Non-Floating Substitution dialog “Next” button pulse when moving between teams. Schedule page uses refs for latest `staffOverrides`/allocations and `mergeStep2Point2StaffOverrides` so Step 2.2 SPT edits never overwrite Step 2.1 substitution overrides; recalc uses source snapshot for immediate Step 3 handoff.
- **Save path** — Save uses `normalizeScheduleStateForSave` so therapist data is always persisted and PCA/extra coverage are only persisted when Step 2 and Step 3 are complete; RPC and fallback save replace PCA rows (delete stale or delete-all-then-insert) so reload does not resurrect old PCA allocations. Migration `20260313_update_save_schedule_v1_replace_pca_rows.sql` updates `save_schedule_v1` to replace PCA allocations.
...
- **“Extra” tag leak before Step 3.4** — `extraCoverageBySlot` is hidden until Step 3.4 is active; display uses `shouldShowExtraCoverage` and schedule page sanitizes overrides with `sanitizeExtraCoverageOverrides` so “Extra” tags do not appear prematurely after Step 2.2.
- **PCA substitution green border (multi-team)** — When a floating PCA substitutes for a non-floating PCA across multiple teams (e.g. MC and GMC), the green border and substitution styling now show for every team that receives coverage; `detectAutoAssignedSubstitutions` considers all allocation teams (base + slot teams) and `derivePcaSubstitutionInfo` centralizes substitution display logic in `PCABlock`.
- **Robotic (and multi-weekday special) 4-slot → 2-slot display** — Step 3 floating availability now excludes slots reserved by Step 2 special-program overrides so the allocator does not reclaim them; for multi-weekday programs, `getSlotTeamForSpecialProgram` re-resolves the runtime model with the schedule weekday so `effectiveRequiredSlots` and slot–team routing are correct and all configured slots are assigned (e.g. Robotic 4 slots → 4 assigned, not 2).
- **Step 2.2 stale closure overwriting Step 2.1** — Applying Step 2.2 SPT final edits no longer overwrites existing Step 2.1 substitution/slot overrides; refs hold latest `staffOverrides`/allocations and `mergeStep2Point2StaffOverrides` merges SPT edits into the current overrides instead of a stale closure.
- **Save then reload: floating PCA in FO and “Extra” tags** — Clearing Step 2 (or beyond) and saving no longer persists baseline-view PCA allocations or extra coverage. Therapist allocations remain after reload (Step 1 complete); PCA allocations are not written when Step 2 or Step 3 is pending, and existing DB PCA rows for the schedule are replaced on save (RPC: delete stale staff IDs; fallback: delete all PCA rows then insert current set) so reload shows therapist-only until Step 3 is run again.

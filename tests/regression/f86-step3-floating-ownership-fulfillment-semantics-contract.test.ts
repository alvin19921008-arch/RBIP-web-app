...
import type { SpecialProgram, PCAPreference } from '../../types/allocation'
...
async function main() {
  ...
  assert.equal(semantics.step2ReservedSpecialProgramCoverageFte, 0.25)
  assert.equal(semantics.substitutionCoverageFte, 0.25)
  assert.equal(semantics.trueStep3FloatingCoverageFte, 0.25)
  assert.equal(semantics.trueStep3FloatingFulfillmentFte, 0.25)
  assert.equal(semantics.postFulfillmentSurplusFte, 0)

  const surplusSemantics = deriveTeamStep3FloatingFulfillmentSemantics({
    team,
    allocations,
    allPcaStaff,
    staffOverrides,
    specialPrograms,
    weekday,
    averagePcaPerTeam: 0.5,
  })

  assert.equal(
    surplusSemantics.trueStep3FloatingFulfillmentFte,
    0,
    'When Step 2 non-floating plus substitution already meets the team target, true Step 3 floating should not be counted as fulfillment.'
  )
  assert.equal(
    surplusSemantics.postFulfillmentSurplusFte,
    0.25,
    'Post-fulfillment surplus should be derived explicitly from true Step 3 floating beyond the remaining need.'
  )
}

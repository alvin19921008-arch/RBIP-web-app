...
import { runGymAvoidanceRepairLoop } from '../../lib/algorithms/floatingPcaV2/repairMoves'
...
function main() {
  ...
  const accepted = runGymAvoidanceRepairLoop({
    teamOrder,
    initialPendingFTE,
    pendingFTE,
    allocations,
    ...
    committedStep3Anchors: undefined,
  })
  assert.ok(accepted >= 1, 'Expected at least one accepted gym-avoidance repair move.')

  const nextPending = computePendingLikeAllocator(initialPendingFTE, baselineAssignedSlots, allocations)
  assert.deepEqual(detectRankedV2GymAvoidableDefects({ ...gymCtx, pendingFTE: nextPending, allocations }), [])
  assert.deepEqual(
    detectRankedV2RepairDefects({
      teamOrder,
      initialPendingFTE,
  ...
  const row = allocations.find((a) => a.staff_id === 'float-a')
  assert.equal(row?.slot4, null, 'FO should no longer occupy configured gym slot 4 with Step-3 floating.')
  assert.ok(row?.slot1 === 'FO' || row?.slot2 === 'FO' || row?.slot3 === 'FO')
}
